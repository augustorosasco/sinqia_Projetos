...
namespace Projeto02_ApiChamados.Controllers.Dados 
{ 
       ...
       public IEnumerable<Chamado> BuscarChamados()
        ...
        public bool RemoverChamado(int id)
        {
            using(var context = new ChamadosContext())
            {
                try
                {
                    var chamado = context.Chamados.FirstOrDefault(p => p.ChamadoId == id);
                    if(chamado.Resolvido == 1)
                    {
                        throw new Exception();
                    }
                    context.Entry<Chamado>(chamado).State = System.Data.Entity.EntityState.Deleted;
                    context.SaveChanges();
                    return true;
        ...
        public bool AlterarChamado(Chamado chamado)
        {
            using (var context = new ChamadosContext())
            {
                ...
                {
                    chamado.Resolvido = 1;
                    context.Entry<Chamado>(chamado).State = EntityState.Modified;
                    context.SaveChanges();
                    return true;
                }
                catch (Exception)
